...
datasets = {'oxford5k', 'oxford105k', 'paris6k', 'paris106k', 'holidays_rotated'};  % 
mask_methods = {'max'};
final_dims = [512, 1024, 4096];

if exist('results/exp6_table6_compare_SOTA_siaMAC_results.mat', 'file')
    load('results/exp6_table6_compare_SOTA_siaMAC_results.mat');
else
...
for dataset_idx=1:length(datasets)
    ...
    % features are stored.
    dataset_name  = [dataset_test, '_', num2str(lid),'_', num2str(max_img_dim), '_siaMAC'];

    dataset_dir   = [data_dir, dataset_name, '/'];
    trainset_dir  = [dataset_dir, dataset_train, '/'];
    baseset_dir   = [dataset_dir, dataset_test, '/'];
    queryset_dir  = [dataset_dir, dataset_test, 'q/'];
    flickrset_dir = [data_dir, 'flickr100k_', num2str(lid),'_', num2str(max_img_dim), '_siaMAC/'];
    ...
    for mask_method_idx=1:length(mask_methods)
        ...
        for final_dim_idx=1:length(final_dims)
            ...
            if (mAPs(dataset_idx, mask_method_idx, final_dim_idx) > 0)
                continue;
            end
    ...
    save('results/exp6_table6_compare_SOTA_siaMAC_results.mat', ...
                        'mAPs', 'datasets', 'mask_methods', 'final_dims');
end
print_table6_compare_SOTA_part2;
